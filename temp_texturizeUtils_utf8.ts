...
import { getContiguousIslands } from './meshUtils';
...
export function applyKnurling(
    ...
    // Detect contiguous surfaces (islands) in the selection
    const islands = getContiguousIslands(geometry, faceIndices);
    if (islands.length === 0) return geometry;
    ...
    islands.forEach(islandIndices => {
        ...
        const pitchP = isPlanar ? targetPitch : (circPhys / nD);
        ...
        for (let t = 0; t < islandIndices.length; t++) {
            let pA_uv = project(points[t * 3]), pB_uv = project(points[t * 3 + 1]), pC_uv = project(points[t * 3 + 2]);
            ...
            for (let j = jS; j <= jE; j++) {
                for (let i = iS; i <= iE; i++) {
                    ...
                    const um = u0 + pU * 0.5, vm = v0 + pV * 0.5;

                    let cell: { u: number, v: number, h: number }[][] = [];

                    if (params.pattern === 'diamond') {
                        const mid = { u: um, v: vm, h: ((Math.abs(i) + Math.abs(j)) % 2 === 0) ? depth : 0 };
                        cell = [
                            [{ u: u0, v: v0, h: 0 }, { u: u1, v: v0, h: 0 }, mid],
                            [{ u: u1, v: v0, h: 0 }, { u: u1, v: v1, h: 0 }, mid],
                            [{ u: u1, v: v1, h: 0 }, { u: u0, v: v1, h: 0 }, mid],
                            [{ u: u0, v: v1, h: 0 }, { u: u0, v: v0, h: 0 }, mid]
                        ];
                    } else if (params.pattern === 'straight' || params.pattern === 'diagonal') {
                        // Ridge along V axis in the middle of U
                        cell = [
                            [{ u: u0, v: v0, h: 0 }, { u: um, v: v0, h: depth }, { u: u0, v: v1, h: 0 }],
                            [{ u: um, v: v0, h: depth }, { u: um, v: v1, h: depth }, { u: u0, v: v1, h: 0 }],
                            [{ u: um, v: v0, h: depth }, { u: u1, v: v0, h: 0 }, { u: um, v: v1, h: depth }],
                            [{ u: u1, v: v0, h: 0 }, { u: u1, v: v1, h: 0 }, { u: um, v: v1, h: depth }]
                        ];
                    } else if (params.pattern === 'square') {
                        // Grid pattern: peaks at mid-edges and a dip at the center
                        const mid = { u: um, v: vm, h: 0 };
                        cell = [
                            [{ u: u0, v: v0, h: 0 }, { u: um, v: v0, h: depth }, mid],
                            [{ u: um, v: v0, h: depth }, { u: u1, v: v0, h: 0 }, mid],
                            [{ u: u1, v: v0, h: 0 }, { u: u1, v: vm, h: depth }, mid],
                            [{ u: u1, v: vm, h: depth }, { u: u1, v: v1, h: 0 }, mid],
                            [{ u: u1, v: v1, h: 0 }, { u: um, v: v1, h: depth }, mid],
                            [{ u: um, v: v1, h: depth }, { u: u0, v: v1, h: 0 }, mid],
                            [{ u: u0, v: v1, h: 0 }, { u: u0, v: vm, h: depth }, mid],
                            [{ u: u0, v: vm, h: depth }, { u: u0, v: v0, h: 0 }, mid]
                        ];
                    }
